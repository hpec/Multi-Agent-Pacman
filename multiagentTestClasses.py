...
import layout, textDisplay, pacman
# import grading
...
class MultiagentTreeState(object):
    ...
    def getScore(self):
        ...
        return self.problem.evaluation[self.state]
...
def run(lay, layName, pac, ghosts, nGames=1, name='games'):
    """
...
class PacmanGameTreeTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        stats = run(lay, self.layout_name, pac, [DirectionalGhost(i + 1) for i in range(2)], name=self.alg)
        if stats['timeouts'] > 0:
...
class EvalAgentTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        disp = graphicsDisplay.PacmanGraphics(1, frameTime=.1)
        #disp = textDisplay.NullGraphics()
